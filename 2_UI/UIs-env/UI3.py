...
from PyQt6.QtGui import QPixmap, QIcon # <-- NUEVO: Importar QIcon
class FluorimetroUI(QMainWindow):
    def __init__(self):
        ...
        icono_path = "./imgs/images.jpg"  
        if os.path.exists(icono_path):
            self.setWindowIcon(QIcon(icono_path))

        # 1. Crear el contenedor y layout principal
        main_widget = QWidget()
        main_layout = QVBoxLayout(main_widget)
        self.setCentralWidget(main_widget)

        # 2. Construir el Encabezado (Logo + Título)
    ...
    def init_header(self, parent_layout):
        ...
        logo_path = "./imgs/images.jpg" # Asegúrate de poner tu imagen con este nombre en la misma carpeta
        
        if os.path.exists(logo_path):
            pixmap = QPixmap(logo_path)
            # Escalar el logo a un alto fijo de 60px manteniendo la proporción
            self.lbl_logo.setPixmap(pixmap.scaledToHeight(60, Qt.TransformationMode.SmoothTransformation))
        else:
            # Marcador de posición por si no encuentra la imagen
            self.lbl_logo.setText(" [ LOGO ] ")
            self.lbl_logo.setStyleSheet("font-weight: bold; color: gray; border: 2px dashed gray; padding: 15px;")
            self.lbl_logo.setAlignment(Qt.AlignmentFlag.AlignCenter)

        # --- Configuración del Título ---
        self.lbl_title = QLabel("Fluorímetro Low Cost")
        self.lbl_title.setStyleSheet("font-size: 28px; font-weight: bold; color: #2C3E50; margin-left: 10px;")

        # Agregar elementos al layout horizontal
        
        header_layout.addStretch() # Empuja el logo y el título hacia la derecha
        header_layout.addWidget(self.lbl_title)
        header_layout.addWidget(self.lbl_logo)
        header_layout.addStretch() # Empuja el logo y el título hacia la izquierda

        # Agregar el encabezado al layout vertical principal
    ...
    def init_gui_tab(self):
        ...
        # NUEVO: Bloquear el límite inferior del eje Y en cero absoluto
        self.graph_widget.getViewBox().setLimits(yMin=0)
        # Opcional: Sugerir un rango inicial de Y para que no empiece aplastado
        self.graph_widget.setYRange(0, 1000) 
